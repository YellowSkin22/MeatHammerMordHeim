...
3. **Storage** (`js/storage.js`) — CRUD over localStorage (key: `mordheim_rosters`), plus import/export as JSON strings. On every `saveRoster()`, fires `Cloud.enqueueSave()` for cloud sync. `_migrateRoster()` runs on every `getAllRosters()` call to silently upgrade old roster shapes (e.g. uppercase stat keys → lowercase, old skill IDs → subtype strings); add new migrations here when the warrior shape changes.
...
```js
{
  ...
  isHiredSword,       // hired swords — skill/spell/equipment access from hiredSwords.json template
...
| File | Purpose |
|------|---------|
| `data/warbandFiles/index.json` | Ordered list of warband file paths with grade; loaded first, then all files fetched in parallel |
| `data/warbandFiles/{grade}/{slug}.json` | Individual warband definitions — hero/henchman templates, stat lines, equipment lists, skill/spell access, `specialRules[].ruleFull` (mined for tooltips at load time) |
...
| `data/version.json` | `{ "version": N }` — bump alongside `?v=N` in `index.html` to force a cache-busting reload on all clients |
